function trilingualDemocracy(group) {
    console.log(group)
  //a counter to keep track of the letter not included.
    let counter = "IDFK"
    for(language of group){
//Check the length of the string without the letter.
        let test =group.replaceAll(language, '')
        if (test.length ==0){return language
//this trips if all three letters are the same.
        }else if (test.length == 1){
//This trips if there were duplicates
            return group.replaceAll(language,'')
        }else {
//Remove the letter from the counter and if there are no duplicates the last letter is the answer
            counter = counter.replace(language,'')
        }
