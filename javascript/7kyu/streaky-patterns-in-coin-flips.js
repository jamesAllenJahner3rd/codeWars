function checkSequence(sequence, l, n) {
    let streakChecker, streakSumer = 0;
    //check if their the letter is the same as the next letter
    for(let index=0;index <= sequence.length -1; index++){
      //The first letter counts as a 1
      if (sequence[index-1] == undefined){
        streakChecker=1
      }else if( sequence[index] !== sequence[index-1]){
        //If the letters don't matchcheck if the counter = l
         if( streakChecker == l){
           //if so increment the sumer
           streakSumer +=1
         }
        //reset the counterb/c the nest letter counts as 1
         streakChecker=1
       }else{
         //other it's streak, increment the counter
         streakChecker+=1
       }
    }
    //if the last streak = l increment the sumer
    if (streakChecker ==l){
        streakSumer +=1
    }
    //check the sumer against l
    return n == streakSumer
  }/*
  Description:
